...
SignalSource (RtlTcpSource | SimulatedSource)
    ...
    ▼
SdrService (@Service)
    │  SpectrumProcessor: Hann window → radix-2 FFT → |·|² → dB → fftshift
    │  paces frames to ~targetFps; socket is always fully drained
    ▼
...
|-------------|-----------------------------------------------------------------------|
| `dsp`       | `Fft`, `Window`, `SpectrumProcessor` — pure-Java signal processing.   |
